  ...
  return (
    ...
    >
      <div className="max-w-6xl mx-auto px-4 sm:px-0">
        ...
        <TextReveal
          as="h2"
          className="text-3xl md:text-5xl font-bold mb-10 sm:mb-12 text-foreground break-words"
          delay={0.1}
        >
          Projects I've Built
        </TextReveal>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6">
          {allProjects.map((project, i) => (
            <ScrollReveal
              ...
              delay={0.1 * (i + 1)}
            >
              <div className="glass rounded-2xl overflow-hidden group hover:glow-border transition-all duration-500 hover:-translate-y-2 flex flex-col h-full">
                {/* Project Image */}
                <div className="h-44 sm:h-48 relative overflow-hidden">
                  ...
                  {/* Overlay */}
                  <div className="absolute inset-0 bg-background/80 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-all duration-400 flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4 px-4">
                    <a
                      href={project.live_url}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="w-full sm:w-auto flex items-center justify-center gap-1.5 px-4 py-2 rounded-lg bg-primary text-primary-foreground text-xs font-medium hover:bg-primary/90 transition-colors"
                    >
                ...
                <div className="p-4 sm:p-6 flex flex-col flex-1">
                  ...
                  <p className="text-muted-foreground text-xs sm:text-sm mb-4 flex-1 leading-relaxed">
                    {project.description}
                  </p>
                  ...
                  <div className="flex flex-wrap gap-2 mb-4 sm:mb-5">
                    {project.tags.map((tag) => (
                      <span
                        key={tag}
                        className="text-xs font-mono px-2.5 py-1 rounded-md bg-secondary text-muted-foreground whitespace-nowrap"
                      >
                        {tag}
                      </span>
                    ))}
                  ...
                  <div className="flex flex-col xs:flex-row gap-2 sm:gap-3">
                    <a
                      href={project.live_url}
                      className="btn-primary text-xs px-4 py-2 flex items-center justify-center gap-1.5 w-full"
                      target="_blank"
                      rel="noopener noreferrer"
                    >
                      <ExternalLink size={14} />
                    ...
                    <a
                      ...
                      className="btn-outline-glow text-xs px-4 py-2 flex items-center justify-center gap-1.5 w-full"
                      target="_blank"
                      rel="noopener noreferrer"
                    >
                      <Github size={14} />
